...
int RecoE906Data(const int nEvents = 1)
{
  ...
  CalibHitElementPos* chep = new CalibHitElementPos();
  //chep->setInputTy(CalibHitElementPos::E906); 
  se->registerSubsystem(chep);
  ...
  //reco->set_evt_reducer_opt("ac"); //if not provided, event reducer will be using JobOptsSvc to intialize; to turn off, set it to "none"
