...
DataAna::DataAna(const std::string& name)
    : SubsysReco(name)
...
int DataAna::Init(PHCompositeNode* topNode)
{
    return Fun4AllReturnCodes::EVENT_OK;
}

int DataAna::InitRun(PHCompositeNode* topNode)
{
...
PHG4Hit* DataAna::FindG4HitAtStation(const int trk_id, const PHG4HitContainer* g4hc)
{
    PHG4Hit* hit = nullptr;
...
std::vector<PHG4Hit*> DataAna::FindG4HitsAtStation(const int trk_id, const PHG4HitContainer* g4hc)
{
...
// find g4shower
PHG4Shower* DataAna::get_primary_shower(PHG4Particle* primary)
{
...
SRecTrack* DataAna::FindBestMomRecTrack(SRecEvent* recEvent, const float true_TargetP)
{
...
int DataAna::FindCommonHitIDs(std::vector<int>& hitidvec1, std::vector<int>& hitidvec2)
{
...
void DataAna::set_legacy_rec_container(bool b)
{
    _legacyContainer = b;
}

void DataAna::save_secondaries(bool b)
{
    _saveSecondaries = b;
}

void DataAna::save_primaries(bool b)
{
    _savePrimaries = b;
}

void DataAna::save_tracks(bool b)
{
    _saveTracks = b;
}

void DataAna::save_vertex(bool b)
{
    _saveVertex = b;
}

int DataAna::ResetEvalVars()
{
    /** Hits
   */
...
int DataAna::process_event(PHCompositeNode* topNode)
{
...
int DataAna::End(PHCompositeNode* topNode)
{
    saveFile->cd();
    saveTree->Write();
    saveFile->Close();
...
int DataAna::GetNodes(PHCompositeNode* topNode)
{
  /*
    ...
    */
    ...
    if (!g4hc_d1x || !g4hc_d3px || !g4hc_d3mx) {
        std::cout << "WARNING:: DataAna::GetNode No drift chamber node " << std::endl;
        //return Fun4AllReturnCodes::ABORTEVENT;
    }
    ...
    if (!g4hc_h1t || !g4hc_h1b || !g4hc_h2t || !g4hc_h2b || !g4hc_h3t || !g4hc_h3b || !g4hc_h4t || !g4hc_h4b) {
        std::cout << "WARNING:: DataAna::GetNode No nodoscope node." << std::endl;
        //return Fun4AllReturnCodes::ABORTEVENT;
    }
    ...
    if (!g4hc_p1y1 || !g4hc_p1y2 || !g4hc_p1x1 || !g4hc_p1x2 || !g4hc_p2x1 || !g4hc_p2x2 || !g4hc_p2y1 || !g4hc_p2y2) {
        std::cout << "WARNING:: DataAna::GetNode No prototube node. " << std::endl;
        //return Fun4AllReturnCodes::ABORTEVENT;
    }

    g4hc_ecal = findNode::getClass<PHG4HitContainer>(topNode, "G4HIT_EMCal");
    if (!g4hc_ecal) {
        std::cout << "WARNING:: DataAna::GetNode No EMcal node" << std::endl;
    }
    return Fun4AllReturnCodes::EVENT_OK;
}

void DataAna::MakeTree()
{
    saveFile = new TFile(saveNameOut, "RECREATE");
    saveTree = new TTree("Events", "Tree Created by DataAna");
