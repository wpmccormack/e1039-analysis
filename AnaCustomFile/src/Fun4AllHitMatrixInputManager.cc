#include "Fun4AllHitMatrixInputManager.h"
...
Fun4AllHitMatrixInputManager::Fun4AllHitMatrixInputManager(const string& name, const string& topnodename)
...
Fun4AllHitMatrixInputManager::~Fun4AllHitMatrixInputManager()
{
  if(isopen) fileclose();
  delete syncobject;
}

int Fun4AllHitMatrixInputManager::fileopen(const string &filenam)
{
  if (isopen) {
  ...
  _tin->SetBranchAddress(_branch_name.c_str(), &hit_matrix);
...
int Fun4AllHitMatrixInputManager::run(const int nevents)
{
readagain:
  if (!isopen) {
    if (filelist.empty()) {
  ...
  int i_hit = 0;
  for (auto i_det = 0; i_det < 30; ++i_det) {
    for (auto i_ele = 0; i_ele < 201; ++i_ele) {
      if (hit_matrix[i_det][i_ele]) {
        SQHit* hit = new SQHit_v1();
        hit->set_hit_id(++i_hit);
        hit->set_detector_id(i_det+1);
        hit->set_element_id (i_ele+1);
        hit->set_tdc_time(0);
...
int Fun4AllHitMatrixInputManager::fileclose()
{
  if (!isopen) {
...
void
Fun4AllHitMatrixInputManager::Print(const string &what) const
{
...
int
Fun4AllHitMatrixInputManager::OpenNextFile()
{
...
int
Fun4AllHitMatrixInputManager::ResetEvent()
{
  syncobject->Reset();
  return 0;
}
...
int
Fun4AllHitMatrixInputManager::PushBackEvents(const int i)
{
...
int
Fun4AllHitMatrixInputManager::GetSyncObject(SyncObject **mastersync)
{
...
int
Fun4AllHitMatrixInputManager::SyncIt(const SyncObject *mastersync)
{
  if (!mastersync) {
